...
describe("createChunkerTransform", () => {
	it.prop(
		[fc.array(bytesArb({ maxLength: 200 }), { maxLength: 10 }), fc.integer({ min: 1, max: 100 })],
		CRYPTO,
	)(
		"emits chunks of at most n, all but the last exactly n, concatenating to the input",
		async (writes, maxChunkSize) => {
			...
			expect(concat(...chunks)).toEqual(concat(...writes));
		},
	);
});
...
describe("chunk-boundary independence", () => {
	it.prop([bytesArb({ maxLength: 512 }), fc.integer({ min: 1, max: 48 }), fc.gen()], CRYPTO)(
		"request: re-splitting the ciphertext stream at arbitrary boundaries still decrypts to the original plaintext",
		async (plaintext, maxChunkSize, g) => {
			...
			// Each delivery shape needs its own recipient context: the context owns
			// the sequence numbering, so it cannot decrypt the same body twice.
			const decryptWithFreshContext = async (pieces: readonly Uint8Array[]) => {
				const serverCtx = await recipientFor(clientCtx);
				return decryptRequestFrames(serverCtx[kRecipientContext], pieces);
			};

			// One giant buffer (a single read).
			expect(await decryptWithFreshContext([cipherBody])).toEqual(plaintext);
			// Byte-at-a-time delivery.
			expect(await decryptWithFreshContext(byteAtATime(cipherBody))).toEqual(plaintext);
			// Arbitrary re-split boundaries, drawn so a failure shrinks with the rest.
			const points = g(splitPointsArb, cipherBody.length, 8);
			expect(await decryptWithFreshContext(splitAt(cipherBody, points))).toEqual(plaintext);
		},
	);

	it.prop([bytesArb({ maxLength: 512 }), fc.integer({ min: 1, max: 48 }), fc.gen()], CRYPTO)(
		"response: re-splitting the ciphertext stream at arbitrary boundaries still decrypts to the original plaintext",
		async (plaintext, maxChunkSize, g) => {
			const cipherBody = await collectStream(
				streamOfBytes(plaintext)
					.pipeThrough(createChunkerTransform(maxChunkSize))
...
describe("truncation is rejected", () => {
	it.prop(
		[fc.array(bytesOfLengthArb(6), { minLength: 1, maxLength: 4 }), bytesOfLengthArb(6), fc.gen()],
		CRYPTO,
	)(
		"any proper prefix of a valid chunked request body fails to decrypt, never yielding partial plaintext",
		async (nonFinalPlaintexts, finalPlaintext, g) => {
			const clientCtx = await client.createRequestContext();

			const nonFinalFrames: Uint8Array[] = [];
			for (const pt of nonFinalPlaintexts) {
				nonFinalFrames.push(frameChunk(await clientCtx.sealChunk(pt), false));
			}
			...
			const cut = g(fc.integer, { min: 0, max: body.length - 1 });

			const serverCtx = await recipientFor(clientCtx);
			const error = await expectOHTTPError(
				decryptRequestFrames(serverCtx[kRecipientContext], [body.subarray(0, cut)]),
			);
	...
	it("dropping just the final chunk (all non-final frames present, no final marker) is InvalidMessage", async () => {
		const { clientCtx } = await freshRequestPair();
		const body = frameChunk(await clientCtx.sealChunk(new Uint8Array([1, 2, 3])), false);
	...
	it("cutting mid-frame (partial length prefix or partial ciphertext) never resolves, and reports InvalidMessage once the stream ends", async () => {
		const { clientCtx } = await freshRequestPair();
		const framed = frameChunk(await clientCtx.sealChunk(new Uint8Array(40)), false);
...
describe("zero-length non-final chunk is rejected", () => {
	it.prop(
		[
			fc.array(bytesOfLengthArb(4), { maxLength: 3 }),
			fc.array(bytesOfLengthArb(4), { maxLength: 3 }),
			bytesOfLengthArb(4),
		],
		CRYPTO,
	)(
		"a deliberately-sealed empty non-final chunk anywhere in the stream fails decryption",
		async (before, after, finalPlaintext) => {
			let counter = 0;
			const frames: Uint8Array[] = [];
			const seal = (pt: Uint8Array, isFinal: boolean) =>
				sealResponseChunk(
					responseKeys.aead,
					responseKeys.aeadKey,
					responseKeys.aeadNonce,
			...
			for (const pt of before) frames.push(frameChunk(await seal(pt, false), false));
			...
			const zeroCt = await seal(new Uint8Array(0), false);
			expect(zeroCt.length).toBeGreaterThan(0); // AEAD tag makes the wire length non-zero
			frames.push(frameChunk(zeroCt, false));

			for (const pt of after) frames.push(frameChunk(await seal(pt, false), false));
...
describe("response chunk counter discipline", () => {
	it.prop(
		[fc.array(bytesOfLengthArb(5), { minLength: 1, maxLength: 5 }), fc.integer({ min: 0, max: 5 })],
		CRYPTO,
	)(
		"frame i needs counter i; a mismatched counter or the wrong AAD fails to open",
		async (plaintexts, wrongOffset) => {
			const n = plaintexts.length;
			const ciphertexts: Uint8Array[] = [];
			for (let i = 0; i < n; i++) {
				ciphertexts.push(await sealChunkAt(i, plaintexts[i] as Uint8Array, i === n - 1));
			}
			...
			for (let i = 0; i < n; i++) {
				...
				expect(await openChunkAt(i, ct, isFinal)).toEqual(plaintexts[i]);

				// A wrong counter fails, whatever the AAD.
				const wrongCounter = (i + 1 + (wrongOffset % Math.max(n, 1))) % (n + 3);
				if (wrongCounter !== i) {
					await expectOHTTPError(openChunkAt(wrongCounter, ct, isFinal));
				}

				// The right counter but the wrong AAD (final vs non-final) fails too.
				await expectOHTTPError(openChunkAt(i, ct, !isFinal));
			}
		},
	);

	it.prop([bytesArb({ maxLength: 256 }), fc.integer({ min: 1, max: 24 })], CRYPTO)(
		"counters produced by the streaming transforms are 0,1,2,... with the final chunk one past the last non-final counter",
		async (plaintext, maxChunkSize) => {
			...
			for (;;) {
				...
				if (vlen === 1 && (header[0] as number) === 0) {
					// Final marker: its ciphertext runs to the end of the body.
					await expect(
						openChunkAt(counter, cipherBody.subarray(offset + vlen), true),
				...
				await expect(
					openChunkAt(counter, cipherBody.subarray(offset + vlen, offset + vlen + length), false),
				).resolves.toBeInstanceOf(Uint8Array);
				offset += vlen + length;
				counter++;
			}
...
describe("computeChunkNonce", () => {
	it.prop([bytesOfLengthArb(12), fc.integer({ min: 0, max: 0xffffffff })], CRYPTO)(
		"XORing the same counter back into a computed nonce recovers the base nonce",
		(base, counter) => {
			expect(computeChunkNonce(computeChunkNonce(base, counter), counter)).toEqual(base);
		},
	);
});
...
describe("frame reordering, duplication, and dropping", () => {
	...
	it.prop(
		[fc.array(bytesOfLengthArb(4), { minLength: 2, maxLength: 4 }), bytesOfLengthArb(4), seedArb],
		CRYPTO,
	)(
		"any non-identity permutation of the non-final frames fails to decrypt",
		async (plaintexts, finalPlaintext, seed) => {
			const { clientCtx, frames, finalFrame } = await sealFrames(plaintexts, finalPlaintext);

			const order = permute(frames.length, seed);
			if (order.every((v, i) => v === i)) return; // skip the rare identity draw
			const body = concat(...order.map((idx) => frames[idx] as Uint8Array), finalFrame);
	...
	it.prop(
		[fc.array(bytesOfLengthArb(4), { minLength: 1, maxLength: 4 }), bytesOfLengthArb(4), seedArb],
		CRYPTO,
	)(
		"duplicating one non-final frame fails to decrypt",
		async (plaintexts, finalPlaintext, seed) => {
			const { clientCtx, frames, finalFrame } = await sealFrames(plaintexts, finalPlaintext);
			...
			await expectOHTTPError(
				decryptRequestFrames(serverCtx[kRecipientContext], [concat(...withDup, finalFrame)]),
			);
		},
	);
	...
	it.prop(
		[fc.array(bytesOfLengthArb(4), { minLength: 2, maxLength: 4 }), bytesOfLengthArb(4), seedArb],
		CRYPTO,
	)("dropping one non-final frame fails to decrypt", async (plaintexts, finalPlaintext, seed) => {
		const { clientCtx, frames, finalFrame } = await sealFrames(plaintexts, finalPlaintext);
		...
		const serverCtx = await recipientFor(clientCtx);
		await expectOHTTPError(
			decryptRequestFrames(serverCtx[kRecipientContext], [concat(...withoutOne, finalFrame)]),
		);
	});
});
...
describe("maxFrameSize enforcement", () => {
	...
	it.prop([fc.integer({ min: maxFrameSize + 1, max: maxFrameSize + 10_000 })], CRYPTO)(
		"a declared non-final length above maxFrameSize is rejected before that many bytes arrive",
		async (declaredLength) => {
			// Only the length prefix is sent - never the (nonexistent) payload -
			// proving the check runs before the frame is buffered.
			const error = await expectOHTTPError(
				decryptResponseFrames([encodeVarint(declaredLength)], maxFrameSize),
			);
			expect(error.code).toBe(OHTTPErrorCode.ChunkLimitExceeded);
		},
	);

	it.prop([fc.integer({ min: maxFrameSize + 1, max: maxFrameSize + 500 })], CRYPTO)(
		"a final chunk whose accumulated length exceeds maxFrameSize is rejected",
		async (overflowLength) => {
			const body = concat(encodeVarint(0), new Uint8Array(overflowLength));
...
describe("ChunkedOHTTPClient/ChunkedOHTTPServer round-trip", () => {
	it.prop(
		...
		async (requestPayload, responsePayload, maxChunkSize) => {
			...
			expect(
				await runClient.decapsulateResponse(createResponseContext, encapsulatedResponse),
...
/**
 * Another matched recipient context for an already-sealed request. The header
 * alone (independent of any Seal calls already issued on the sender) is all
 * SetupRecipient needs.
 */
function recipientFor(clientCtx: { header: Uint8Array }) {
	return server.createRequestContext(clientCtx.header);
}

/** Seal `plaintexts` as non-final frames plus a final frame, on one fresh context. */
async function sealFrames(plaintexts: readonly Uint8Array[], finalPlaintext: Uint8Array) {
	const { clientCtx } = await freshRequestPair();
	const frames: Uint8Array[] = [];
	...
	return {
		clientCtx,
		frames,
		finalFrame: frameChunk(await clientCtx.sealFinalChunk(finalPlaintext), true),
	};
}

/** Seal one response chunk at an explicit counter, under the shared response keys. */
function sealChunkAt(
	counter: number,
	plaintext: Uint8Array,
	isFinal: boolean,
): Promise<Uint8Array> {
	return sealResponseChunk(
		responseKeys.aead,
		responseKeys.aeadKey,
		responseKeys.aeadNonce,
		counter,
...
/** Open one response chunk at an explicit counter, under the shared response keys. */
function openChunkAt(
	counter: number,
	ciphertext: Uint8Array,
	isFinal: boolean,
): Promise<Uint8Array> {
	return openResponseChunk(
		responseKeys.aead,
		responseKeys.aeadKey,
		responseKeys.aeadNonce,
		counter,
...
function decryptResponseFrames(
	pieces: readonly Uint8Array[],
	maxFrameSize: number = DEFAULT_MAX_FRAME_SIZE,
): Promise<Uint8Array> {
	return collectStream(
