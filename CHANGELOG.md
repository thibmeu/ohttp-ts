...
- Fixed delays in chunked requests and responses when the sender pauses between chunks.
...
- Updated to `bhttp-ts` 0.5.3 so chunked Fetch APIs preserve backpressure and cancellation end to end, keeping plaintext read-ahead bounded when the network consumer stalls without changing the public API.
