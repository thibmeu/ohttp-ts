...
- `KeyConfigWithPrivate` carries `suites`, a list, where it carried a single `suite`. `KeyConfig.generate`, `.derive`, and `.import` take one suite or several, and a gateway can now serve the RFC 9458 Appendix A shape: several `(KDF, AEAD)` pairs under one key identifier, decrypted with the suite the request header names. `KeyConfig.selectSuite` performs that lookup. The suites must share a KEM, since the config advertises one, and one key pair covers them all.
- The `symmetricAlgorithms` argument is gone from the three constructors. It is derived from the suites, so passing it could only agree or lie, and a pair with no suite behind it is one the gateway advertises, accepts in a request header, and then fails to open. The field on `KeyConfig` stays: it is the wire format. Both servers re-derive it at construction, since a spread of a `KeyConfig` object walks past the constructors, and they copy the array they are given.
- `responseCrypto.kdf` and `.aead` accept a list of factories as well as one. The suite is chosen per request now, while `responseCrypto` is fixed at construction, so a gateway serving two AEADs under one key identifier needs a factory for each; with a single override it served one pair and failed the other with `UnsupportedCipherSuite` after the request had already decrypted. An override that covers none of a suite's algorithms still throws rather than falling back.
- Suites in one key config are checked against the key pair itself, not just their KEM id. `hpke`'s WebCrypto X25519 and `@panva/hpke-noble`'s share an id while their keys are not interchangeable, so mixing them published a pair whose requests failed at `SetupRecipient`.
- Both servers reject a key config list that is empty or repeats a key identifier. Lookup is by identifier alone, so a duplicate shadows every later config sharing it and a botched rotation surfaced as `UnsupportedCipherSuite` on live requests.
...
- Private keys in a `KeyConfigWithPrivate` are not extractable. `generateKeyConfig` takes a trailing `extractable` argument, default `false`, for when the library holds the only copy of the key; `deriveKeyConfig` and `importKeyConfig` never export, since the caller supplied the material.
- `generateKeyConfig`, `deriveKeyConfig`, and `importKeyConfig` reject a non-integer `keyId`. `NaN`, which is what `Number(process.env.KEY_ID)` yields for an unset variable, passed both range comparisons and produced a config no gateway could publish.
- A malformed inner binary HTTP stream now cancels the stream it was reading. A rejected `pull()` errors the stream without running its `cancel()`, so the upstream decrypting stream stayed open on input a peer controls.
